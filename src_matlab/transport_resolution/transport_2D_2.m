classdef transport_2D_2
    ...
    methods(Access=public)
        ...
        function obj=transport_2D_2(t,N)        
            obj.t=t;
            obj.N=N;
        end
...
%                 spacing_=30*24*3600;
        ...
        function obj=compute_trajectories(obj,velocity,block_size,x_S,x_Q,integrated_parsec,hydraulic_head,speed_option,folder_mex_option)
                    ...
                    for i=1:(length(obj.t)-1)
                        ...
                        if(sum(Bool_inj_spat)~=0) 
                            if(sum(Bool_inj_nonsat)~=0)
                                ...
                                [~,traj_temp_nonsat] = ode45(Velocity_2D,obj.t(i:end),[x_S(Bool_inj_nonsat);hydraulic_head(Bool_inj_nonsat,i)],options_reg);
                            end
                            % compute trajectories on saturated areas
                            traj_temp_sat=[x_S(Bool_inj_sat);hydraulic_head(Bool_inj_sat,i)]';
                            ...
                            mat_pos_allocate_z{i}=[matrix_positions(~bool_delete,:),z_traj_temp(~bool_delete)];%mat_pos_allocate(compt:compt+sum(~bool_delete)-1,:)=[matrix_positions(~bool_delete,:),x_traj_temp(~bool_delete)];%=[mat_pos_allocate_x;[matrix_positions(~bool_delete,:),x_traj_temp(~bool_delete)]];%
                        end
...
%                 seepage_proportion=deval(sol_simulated,t,2*block_size+2:3*block_size+1)
        ...
        % analyze particle paths present in the same element of the hillslope at a given time. If seepage is occuring in this element
        % then a certain number of particle is exiting the medium according to the ratio of seepage vs total flow the element is experiencing
        function [obj,Error_RF_DGW,Error_ET]=cut_trajectory_ET_RF(obj,block_size,x_S,x_Q,Flux_in_spat,RF_spat,speed_option)
...
%                 end
            ...
            [I_z,J_z,S_z]=find(obj.z_traj);
            keep_z=J_z<=Position_to_tag_before_deletion2(I_z,2);
            obj.z_traj=sparse(I_z(keep_z), J_z(keep_z), S_z(keep_z) ,size_row,size_column);
...
% % %#20180116                         [r,c] = find(~isnan(x_traj_temp));
        ...
        function [t_out,delta_t,x_fin,delta_x_groundwater,t_in,weights]=get_trajectory_properties(obj)
...
%         end
        ...
        function [obj,t_out_groundwater,transit_times_groundwater,x_fin_groundwater]=transport_with_rooting(runs,x,slope_angle,k_soil)
            ...
            % instantiate transport_2D_2 object
            [obj,x_S,x_Q,width,velocity,f]=transport_2D_2.instantiate_transport_and_velocity_field(runs,DPSA_prop,DGW_prop);
            block_size=length(x_S);
            %             dx=x_Q(2:end)-x_Q(1:end-1);
...
%             integrated_parsec(end,:)=0;
            hydraulic_head=bsxfun(@rdivide,runs.simulation_results.S,f.*width);              
%             Velocity_and_Integrated_Parsec=[velocity;integrated_parsec];

            % option 1 all integrated
            speed_option='slow';
            obj=obj.compute_trajectories(velocity,block_size,x_S,x_Q,integrated_parsec,hydraulic_head,speed_option);%compute_trajectories2(velocity,block_size,x_S,x_Q,Bool_sat);
            if(sum(RF_spat(:)<0)>0)
...
%             [t_out_groundwater,transit_times_groundwater,x_fin_groundwater]=obj.get_trajectory_properties;
            [t_out_groundwater,transit_times_groundwater,x_fin_groundwater,delta_x_groundwater,t_in,weights]=obj.get_trajectory_properties;
...
% % %             transit_times_soil(obj.ET>0)=nan;
        ...
        function transport_2compartments(run_shallow,run_deep)
            obj=transport_2D_2.run_transport_simu(run_shallow);
            % retrieve the particles essential properties for their travel inside the aquifer
            [t_out_groundwater,transit_times_groundwater,x_fin_groundwater]=obj.get_trajectory_properties;
        ...
        function [obj_shallow,obj_reg,obj_soil,time_support,weighted_pdfs,weighted_pdfs_shallow,weighted_pdfs_deep_approx,...
            ...
            tic
            obj_soil=transport_2D_2.run_transport_simu(run_soil);
            toc
            [obj_reg,DPSA_shallow,RF_shallow,x_S]=transport_2D_2.run_transport_simu(run_reg);
            toc
...
%             [obj_deep,DPSA_deep,RF_deep]=transport_2D_2.run_transport_simu(run_bed);
%             toc
%             % transport total
%             obj_tot=obj_deep;
...
% %                 median_tot(i)=time_support(Index_)/(3600*24);
        ...
        function [obj,x_S,x_Q,width,velocity,f]=instantiate_transport_and_velocity_field(runs,DPSA_prop,GW_prop)
            sim_res=runs.simulation_results;
            bouss_sim=runs.boussinesq_simulation;
            %             f=bouss_sim.hydraulic_properties.f;
            t=sim_res.t;
            N_in=bouss_sim.compute_recharge(t);
            [~,width,~,~,~,f]=bouss_sim.discretization.get_resampled_variables;
            x_S=sim_res.x_S;
            x_Q=sim_res.x_Q;
            dx=x_Q(2:end)-x_Q(1:end-1);
        ...
        function [obj,DPSA_shallow,RF_shallow,x_S]=run_transport_simu(run_boussinesq)
            [obj,x_S,x_Q,width,velocity,Bool_sat,sol_simulated]=transport_2D_2.instantiate_transport_and_velocity_field(run_boussinesq);
            block_size=length(x_S);
...
%                     output_list_directory{compt}=fullfile(directory,directory_list{i},A.name);
                    ...
                    clear bouss_sim obj
                    transport_2D_2=transport_2D_2.test(runs1);
                    save(fullfile(directory,directory_list{i},'transport_2D_2.mat'),'-v7.3');
                    clear transport_2D_2 runs1
                end
            end
        end
