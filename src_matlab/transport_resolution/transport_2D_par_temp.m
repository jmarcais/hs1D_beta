classdef transport_2D_par_temp
    ...
    methods(Access=public)
        ...
        function obj=transport_2D_par_temp(t,N)        
            obj.t=t;
            obj.N=N;
        end
...
%                 [ttd_arrays{i},transit_times{i},weights_{i},~,~,number_of_particle(i)]=obj.get_rtd(obj.t(i));
        ...
        function [DPSA_prop,DGW_prop,DPSA,RF_spat,Flux_in_spat]=compute_DPSA_GW_proportion(obj,runs,steady_opt)
            if(nargin<3)
                steady_opt='off';
            end
            [DPSA,~,DPSA_spat,RF_spat]=compute_DPSA_RF(runs.simulation_results,runs.boussinesq_simulation,steady_opt);
            [~,Flux_in_spat]=compute_infiltration(runs.simulation_results,runs.boussinesq_simulation,steady_opt);
            DPSA_spat(1,:)=1;
...
%                 fprintf(strcat('WARNING: \n',num2str(sum(DPSA_prop>1)),' values in the spatialized matrix representing Direct Precipitations on Saturated Areas \n','were above the Flux_in infiltration matrix values \n','(maximal value was ',num2str(percent_above),'%% above the corresponding infiltration value). \n','They have been replaced by the corresponding infiltration value.\n'));
        ...
        function [BF_prop,RF_prop,ET_prop,BF,RF,ET]=compute_BF_RF_ET_proportion(obj,RF_spat,Flux_in_spat)
            ...
            RF_prop=RF./(ET+RF+BF);
            ET_prop=ET./(ET+RF+BF);
            BF_prop(ET+RF+BF<=0)=0;
            RF_prop(ET+RF+BF<=0)=0;
            ET_prop(ET+RF+BF<=0)=0;
       end
...
%             f=bouss_sim.hydraulic_properties.f;
            ...
            obj=transport_2D_par_temp(t,N_in);
            obj.x=x_S;
%             spacing_=1*24*3600;
            spacing_=-1;
...
%         end
        ...
        function [obj,mat_pos_allocate_x_z,t_out,transit_times,distance,weights,t_in,x_fin,x_init,Flowpaths_id,DPSA_part,GW_part,Seep_part,DGW_part,ET_part]=transport_main(hs1D_run,phi_tot,steady_opt)
            if(nargin<3)
                steady_opt='off';
            end
            threshold=0;
            % instantiate transport_2D_par_temp object
            [obj,x_S,x_Q,width,velocity,DPSA_,RF_spat,Flux_in_spat,integrated_parsec,hydraulic_head]=transport_2D_par_temp.instantiate_transport_and_velocity_field(hs1D_run,threshold,steady_opt);
            block_size=length(x_S);
            if(nargin>2 && strcmp(steady_opt,'on'))
                obj.t_inj=obj.t_inj(1);
                obj.t_inj_pos=obj.t_inj_pos(1);
                obj.N_inj=obj.N_inj(1);
                obj.weight=obj.weight(1:block_size);
                obj.ET=obj.ET(1:block_size);
                obj.DGW=obj.DGW(1:block_size);
                obj.RF=obj.RF(1:block_size);
                obj.DPSA=obj.DPSA(1:block_size);
            end
...
%             toc
            %% option 2
            if(nargin>1)
                [~,~,~,~,~,~,~,f_edges]=hs1D_run.boussinesq_simulation.discretization.get_resampled_variables;
                transit_times=transit_times*phi_tot./unique(f_edges);
                t_in=t_out-transit_times;
            end
...
%             end
            ...
            if(nargin>2 && strcmp(steady_opt,'on'))
                t_in=max(transit_times)-transit_times;
                t_out=max(transit_times)*ones(size(transit_times));
            end
...
%2022             [DPSA_part,GW_part]=obj.check_particle_tracking_flux_conservation(t_out,weights,DPSA_,RF_spat,plot_option);
            RF_=sum(RF_spat(2:end,:));
            BF_=RF_spat(1,:);
            if(strcmp(steady_opt,'on'))
                DPSA_part=sum(weights(Flowpaths_id==1)/1000)/(mean(obj.t(2:end)-obj.t(1:end-1)));
                Seep_part=sum(weights(Flowpaths_id==2)/1000)/(mean(obj.t(2:end)-obj.t(1:end-1)));
                DGW_part=sum(weights(Flowpaths_id==3)/1000)/(mean(obj.t(2:end)-obj.t(1:end-1)));
                ET_part=sum(weights(Flowpaths_id==4)/1000)/(mean(obj.t(2:end)-obj.t(1:end-1)));
                GW_part=DGW_part+Seep_part;
            else
        ...
        function transport_2compartments(run_shallow,run_deep)
            obj=transport_2D_par_temp.run_transport_simu(run_shallow);
        ...
        function [obj_shallow,obj_reg,obj_soil,time_support,weighted_pdfs,weighted_pdfs_shallow,weighted_pdfs_deep_approx,...
            ...
            tic
            obj_soil=transport_2D_par_temp.run_transport_simu(run_soil);
            toc
            [obj_reg,DPSA_shallow,RF_shallow,x_S]=transport_2D_par_temp.run_transport_simu(run_reg);
            toc
...
%             [obj_deep,DPSA_deep,RF_deep]=transport_2D_par_temp.run_transport_simu(run_bed);
%             toc
%             % transport total
%             obj_tot=obj_deep;
...
% %                 median_tot(i)=time_support(Index_)/(3600*24);
        ...
        function [obj,x_S,x_Q,width,velocity,DPSA,RF_spat,Flux_in_spat,dvzdz,hydraulic_head]=instantiate_transport_and_velocity_field(runs,threshold,steady_opt,phi_tot)
            if(nargin<2)
                threshold=0;
            end
            if(nargin<3)
            ...
            [DPSA_prop,GW_prop,DPSA,RF_spat,Flux_in_spat]=obj.compute_DPSA_GW_proportion(runs,steady_opt);
...
%                 fprintf(strcat('WARNING: \n',num2str(sum(DPSA_prop>1)),' values in the spatialized matrix representing Direct Precipitations on Saturated Areas \n','were above the Flux_in infiltration matrix values \n','(maximal value was ',num2str(percent_above),'%% above the corresponding infiltration value). \n','They have been replaced by the corresponding infiltration value.\n'));
            ...
            % instantiate transport_2D_par_temp object
            [obj,x_S,x_Q,width,velocity]=transport_2D_par_temp.instantiate_transport_and_velocity_field(run_boussinesq,DPSA_prop,DGW_prop,threshold);
            block_size=length(x_S);
...
%                     output_list_directory{compt}=fullfile(directory,directory_list{i},A.name);
                    ...
                    clear bouss_sim obj
                    transport=transport_2D_par_temp.test(runs1);
