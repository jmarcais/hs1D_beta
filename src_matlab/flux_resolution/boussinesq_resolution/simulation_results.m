...
%             Outflow_river=Outflow_river/Watershed_area*1e3*24*3600; % in mm/d
            ...
            Outflow_river=Seepage_flux_hillslope+Subsurface_flux;
...
%             legend('Subsurface Flow','Total Outflow (subsurface and seepage)')
        ...
        function save_velocity_field(obj,filename)
%             Q_cell_centre=interp1(obj.x_Q,obj.Q,obj.x_S);
%             S_reg=obj.S+1e-3.*double(obj.S<=0);
%             Velocity=Q_cell_centre./S_reg;
%             Velocity=[zeros(size(obj.t));Velocity];
%             Velocity_2=nan(size(Velocity)+1);
%             Velocity_2(1,:)=[nan, obj.t];
%             Velocity_2(:,1)=[nan;-1; obj.x_S];
%             Velocity_2(2:end,2:end)=Velocity;
            Q_cell=obj.Q;
            S_reg=obj.S+0.*double(obj.S<=0);
            S_reg=(S_reg(1:end-1,:)+S_reg(2:end,:))/2;
            S_reg=S_reg+1e-3.*double(S_reg<=0);
            S_reg=[ones(1,length(S_reg));S_reg;ones(1,length(S_reg))];
            Velocity=Q_cell./S_reg;
            Velocity_2=nan(size(Velocity)+1);
            Velocity_2(1,:)=[nan, obj.t];
            Velocity_2(:,1)=[nan; obj.x_Q];
            Velocity_2(2:end,2:end)=Velocity;
            dlmwrite(fullfile(filename,'velocity_field.txt'),Velocity_2, '-append', 'precision', '%E','delimiter','\t');
            save(fullfile(filename,'velocity_field.mat'),'Velocity_2');
        end
...
%                 k_soil=coef_soil_perm*k;
            ...
            t2=bsxfun(@times,ones(size(obj.QS)),obj.t);
            t2=bsxfun(@plus,t2,travel_time_soil);
            
            size_t=size(t2);
            QS2=nan(size(obj.QS));
            for i=1:size_t(1)
                QS2(i,:)=interpn(t2(i,:),obj.QS(i,:),obj.t);
            end
        ...
        function Q_river=compute_river_flow_with_rooting(obj,k,slope_angle,coef_soil_perm)
            Q_river=-obj.Q(2,:);
            if(nargin<4)
                QS_out=obj.compute_seepage_spatialized_with_rooting(k,slope_angle);
            else
                QS_out=obj.compute_seepage_spatialized_with_rooting(k,slope_angle,coef_soil_perm);
            end
            QS_out=QS_out(2:end,:);
            Q_river=Q_river+nansum(QS_out);
        end
...
%             Bool_saturated=Bool_saturated(2:end,:);
            Flux_in_spat=Watershed_area_spatialized*Flux_in;
        ...
        function [Flux_in,Flux_in_spat]=compute_infiltration(obj,bouss_sim)
            Flux_in_spat=bouss_sim.compute_source_term_spatialized_vectorized(obj.t,obj.S);
            dx=obj.x_Q(2:end)-obj.x_Q(1:end-1);
            dx_QS=diag(dx);
