classdef simulation_set3
    ...
    methods(Access=public)
        function obj=simulation_set3(mother_folder_directory)
        ...
        % find directories in simulation_set3 that share the same infiltration chronicle
        ...
        % find directories in simulation_set3 that share the same hillslope shape
        ...
        function obj=run_simulation_set3(obj)
...
%                         qs1=bsxfun(@rdivide,qs1,w_1.*dx_Q);
    ...
    methods(Static)
        function run_simulations(folder_root)
            sim_set=simulation_set3(folder_root);
            sim_set=sim_set.instantiate_all_inputs_directory;
            sim_set=sim_set.run_simulation_set3;
        end
        
        function rerun_simulation(SimulationPath)
            sim_set=simulation_set3(SimulationPath);
       ...
       function [run_bedrock,run_regolith]=simulation_3comp_oxy_anoxy(f_bed,k_bed,f_reg,k_reg,file_path)
             tic
...
%                 d_init_add=2.7707;
                ...
                elseif(strcmp(file_path(occurence_slash(end)+1:end-4),'Ris'))
                  ...
                  obj=simulation_set3(folder_root);
                ...
                slope_bottom_regolith=((linspace(0.1^3,1^3,length(x))).^0.333)'.*slope_angle;%(mean(slope_bottom_soil)-0.04); % instead of 0.04 -> to get the 8m regolith depth on average on the hillslope
                z_bottom_regolith=z_land_surface(1)+cumtrapz(x,slope_bottom_regolith.*ones(size(x)))-1; % chose the slope such as in average z_land_surface-z_top=8m
                
                slope_bottom_bedrock=0*slope_angle;%(mean(slope_bottom_regolith)-0.1)*ones(size(x));%
                z_bottom_bedrock=z_bottom_regolith(1)+cumtrapz(x,slope_bottom_bedrock)-d_init_add+1;
                d_bed=z_bottom_regolith-z_bottom_bedrock;
                d_reg=z_land_surface-z_bottom_regolith;
...
%                 plot(t,RF_bed+RF_soil+DPSA_soil)
        ...
        function simulation_2compartments(k_bed,f_bed,k_soil,f_soil,file_path)
            ...
            [~,~,Q_month,~,run_obj]=simulation_set3.run_simulation_rooting(k_bed,soil_coef,file_path,f_bed);
            [x_S1,w_1,d1_2,angle1,x_Q1,f1,k1_2]=get_resampled_variables(run_obj.boussinesq_simulation.discretization);
...
%             end
            ...
            occurence_slash=strfind(file_path,'\');
            folder_root=strcat(file_path(1:occurence_slash(end-1)),file_path(occurence_slash(end)+1:end-4));
            obj2=simulation_set3(folder_root);
...
%             DSi_out=sum(DSi,1); 
        ...
        function [residual_tot,k,d,time_aqui,DPSA_prop]=cost_function(x,file_path)
            ...
            load(file_path);
%             f=x(1); k=x(2);
%             Q_out=simulation_set3.run_simulation(f,k);
            %f=x(1); k=x(2); d1=x(3); d2=x(4);
%             k=x(1); d1=x(2); d2=x(3);
%             [Q_out,DSi_out]=simulation_set3.run_sim_transport(k,d1,d2);
            if(length(x)==2)
                k=x(1); d=x(2);
            else
                k=x(1); d=0;
            end
            [Q_out,time_aqui,DPSA_prop]=simulation_set3.run_simulation3(k,d,file_path);%,nanmean(Q_real));
...
%             plot(t_real,Q_out);
        ...
        function [residual_tot,k,d]=cost_function2(x,file_path)
            ...
            k=x(1); d=x(2); f=x(3);
            Q_out=simulation_set3.run_simulation3(k,d,f);
...
%             f=nan(size_x(1),length(file_path));
            ...
            for j=1:2 %1:length(file_path)
                ...
                figure; hold on
                for i=1:size_x(1)
                    [res(i,j),k(i,j),d(i,j),time_aqui(i,j),DPSA_prop(i,j)]=simulation_set3.cost_function(x(i,:),file_path{j});
                    %                 [res(i),k(i),d(i)]=simulation_set3.cost_function2(x(i,:));
...
%             cost_func=@(x) simulation_set3.cost_function(x);
%             f_init=0.3;
%             k_init=0.07;
...
%             f_opt=x_opt(1);
        ...
        function [k_opt,residual]=optimize_simu2(x_init,file_path)
            cost_func=@(x) simulation_set3.cost_function(x,file_path);
%             x_init=0.01;
...
%             f_init=[0.1,0.2,0.3,0.4];
            ...
            for i=1:length(k_init)
                [k_opt(i),residual(i)]=simulation_set3.optimize_simu2(k_init(i),file_path{i});
            end
        end
    end
end